...
export async function POST(req: Request) {
    try {
        ...
        // --- ACCIÓN: CORREGIR MONTO DE TICKET Y PAGO ---
        if (action === "corregir_ticket_monto" || action === "actualizar_monto_ticket") {
            const ticketId = body.ticketId || body.id || 'H9NUO560';
            const nuevoMonto = parseFloat(body.monto || '300') || 300;
            ...
            const res = await prisma.$transaction(async (tx) => {
                ...
                let saldoFinal = 0;
                if (pago) {
                    const saldoAnterior = parseFloat(pago.saldoAnterior.toString());
                    saldoFinal = Math.max(0, saldoAnterior - nuevoMonto);
                ...
                return { ticketId, nuevoMonto, saldoFinal };
            });

            return NextResponse.json({
                success: true,
                message: "Monto de ticket y pago corregido exitosamente",
                ...res
            });
        }
